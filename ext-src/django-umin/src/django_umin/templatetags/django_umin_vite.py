...
VITE_DEV_SERVER_HOST = getattr(settings, "DJANGO_UMIN_VITE_DEV_SERVER_HOST", "localhost")
...
def vite_asset(path, app):
    ...
    if VITE_DEV_MODE:
        ...
        # Get the path to the app directory relative to the project root
        app_path_relative_to_project = os.path.relpath(app_config.path, settings.BASE_DIR)

        # Construct the asset path relative to the project root
        asset_url_path = os.path.join(app_path_relative_to_project, 'fe', path)

        # The Vite dev server will serve the file from this path
        full_asset_url = f"{base_url}/{asset_url_path}"
    ...
    manifest_path = os.path.join(
        settings.STATIC_ROOT, app, "dist", "manifest.json"
    )
    ...
    if not os.path.exists(manifest_path):
        ...
        if not os.path.exists(app_static_manifest_path):
            raise RuntimeError(f"Vite manifest not found. Looked in {manifest_path} and {app_static_manifest_path}. Did you run `manage.py vite_build`?")
